...
def train_epoch(model, training_data, optimizer, opt):
    ...
    for batch in tqdm(training_data, mininterval=2,
        ...
        p_loss = 0
        for i in range(len(opt.type_z)):
            
            z, y = opt.type_z[i], opt.type_y[i]
            z_score, y_score = zyscores(model, output[:,:-1,:], event_type, z, y )
            # B excites A
            if opt.sle == 0 and opt.excites == 1: # we need penalize effect or non excites if nonexcites produces large probability for every instance in a sequence
              loss_cp =  F.relu((event_type!=z)[:,:-1]* y_score[:,1:]/(1.0-z_score[:,:-1])- (event_type==z)[:,:-1] * y_score[:,1:]/z_score[:,:-1])
              loss_cp = torch.sum(loss_cp)
            elif opt.sle == 0 and opt.excites == 0:
              loss_cp =  F.relu( (event_type==z)[:,:-1] * y_score[:,1:]/z_score[:,:-1] - (event_type!=z)[:,:-1]* y_score[:,1:]/(1.0-z_score[:,:-1]) )
              loss_cp = torch.sum(loss_cp)
            elif opt.sle == 1 and opt.excites == 1:
              loss_cp =  torch.mean((event_type!=z)[:,:-1]* y_score[:,1:]/(1.0-z_score[:,:-1])- (event_type==z)[:,:-1] * y_score[:,1:]/z_score[:,:-1], dim=-1)
              loss_cp = torch.sum(F.relu(loss_cp))
            else:
              loss_cp =  torch.mean( (event_type==z)[:,:-1] * y_score[:,1:]/z_score[:,:-1]- (event_type!=z)[:,:-1]* y_score[:,1:]/(1.0-z_score[:,:-1]), dim=-1)
              loss_cp = torch.sum(F.relu(loss_cp))
...
def log_likelihood_event(model, data, types, event_interest):
    ...
    ll = (event_log_ll + nonevent_log_ll)*non_pad_mask
    ll2 = torch.sum(ll)
...
def main():
    ...
    parser.add_argument('-type_z', type=int, nargs='+', help='List of items', default= [5])
    parser.add_argument('-type_y', type=int, nargs='+', help='List of items', default= [8])
    
    # sequence level effect ==> 1 or instance level effect ==> 0
    parser.add_argument('-sle', type=int, default=1)
    # excites effect ==> 1, inhibits effect ==> 0
    parser.add_argument('-excites', type=int, default=1)
    ...
    best_model = train(model, trainloader, devloader, testloader, optimizer, scheduler, opt, event_interest=None)

    model.load_state_dict(best_model)

    model.eval()

    valid_ll = eval_epoch(model, devloader, opt, event_interest=None)

    test_ll = eval_epoch(model, testloader, opt, event_interest=None)
